...
function monthGridHTML(year, month, markerFn, dayAttrFn, allDaysClickable){
  ...
  const dayCells = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    if (marker.color || allDaysClickable){
      const cls = 'month-overview-day-num' + (marker.color ? ' month-overview-day-num-filled' : '');
      return `
...
function monthOverviewBlockHTML(year, month){
  ...
  const gridInner = monthGridHTML(
    ...
    day => `data-day-popup="${year}-${month}-${day}" aria-label="Training am ${day}.${month+1}. anzeigen"`,
    false
  );
